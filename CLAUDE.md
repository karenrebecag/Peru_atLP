...
Landing page versionada para **ATFX Perú**. Lógica + estilos servidos vía jsDelivr;
**Elementor solo aporta un punto de montaje**. Build: esbuild + TypeScript + GSAP.

Origen: duplicado de `ATOM_Academy` (arquitectura vanilla + Mount Point Pattern), git
desconectado. Sobre esa base trasladamos el **diseño y la maquetación de SparkSummit2026**.

## Por qué este proyecto existe

`SparkSummit2026` es un Next.js (React + Tailwind v4 + GSAP + Lenis) que NO se puede
incrustar en Elementor tal cual. Para conservar su diseño sin rehacer la etapa de diseño,
**trasladamos sus secciones y layouts 1:1 a este stack vanilla**. Se replica estructura,
spacing, tokens y animaciones; el contenido/copy se reemplaza por el de ATFX Perú.

- Referencia de diseño: `/Users/karenrebecaog/Desktop/SoftwareDevProjects/SparkSummit2026`
- Regla: **layout y maquetación idénticos**, contenido propio de ATFX Perú.

## Identidad (PENDIENTE de rebrand)

| Campo | Valor |
|---|---|
| Repo | _TBD_ (debe ser público — requisito de jsDelivr `/gh/`) |
| Remote | _TBD_ |
| CDN loader | `https://cdn.jsdelivr.net/gh/<owner>/<repo>@latest/loader.js` |
| Bundle JS | `dist/landing.js` |
| Bundle CSS | `dist/landing.css` |

> Aún apuntan a `karenrebecag/Academy_LP` en `loader.js`, `src/index.ts` y refs internas.
> Limpiar al definir el repo destino. Prefijos `--aa-*` / `.aa-*` se conservan (scoping ya
> probado; solo se remapean los **valores** de los tokens a la paleta de SparkSummit).
...
<script data-cfasync="false"
  src="https://cdn.jsdelivr.net/gh/<owner>/<repo>@latest/loader.js"></script>
```
